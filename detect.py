    ...
    for r in results:
        ...
        for box in boxes:
            ...
            if currentClass in ["car", "motorbike", "truck", "bus"] and conf > 0.35:
                cvzone.cornerRect(img, (x1, y1, w, h),t=1,rt=2,l=5,colorR=(0,255,0))
                currentArray = np.array([x1, y1, x2, y2, conf])
                detections = np.vstack((detections, currentArray))
                cvzone.putTextRect(img, f'{classNames[cls]} {conf}', (max(0, x1), max(35, y1)), scale=1, thickness=2,offset=3,border=1)
